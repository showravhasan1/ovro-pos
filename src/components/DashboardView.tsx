...
export default function DashboardView({ onBackToPos }: DashboardViewProps) {
            ...
            <header className="bg-white border-b border-zinc-200 px-6 py-4">
                <div className="max-w-7xl mx-auto flex justify-between items-center">
                    <div className="flex items-center gap-4">
                        <button onClick={onBackToPos} className="text-xl font-black text-zinc-900 flex items-center gap-2">
                            <ArrowLeft size={20} className="text-zinc-500" />
                            Ovro<span className="text-blue-600">POS</span>
                        </button>
                        <span className="text-zinc-300">|</span>
                        <h1 className="text-base font-bold text-zinc-600">Owner Dashboard</h1>
                    </div>
            ...
            <main className="max-w-7xl mx-auto p-6 space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {stats.map((stat, i) => (
                        <div
                            key={i}
                            className={`bg-white p-5 rounded-2xl border border-zinc-200 ${stat.label.includes('Low Stock') ? 'cursor-pointer hover:border-red-300 transition-colors' : ''
                                }`}
                            onClick={() => {
                                if (stat.label.includes('Low Stock')) {
                                    window.alert('Please go to Stock page to view low stock items.');
                                }
                            }}
                        >
                            ...
                            <div className={`text-xs mt-2 flex items-center gap-1 ${stat.positive ? 'text-green-600' : 'text-red-500'}`}>
                                ...
                                {stat.label.includes('Low Stock') && (
                                    <span className="ml-2 text-blue-600 font-medium">View in Stock</span>
                                )}
                            </div>
                        </div>
                    ))}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="space-y-3">
                            {topItems.map((item, i) => (
                                <div key={i} className="flex items-center gap-4">
                                    <span className="w-8 h-8 flex items-center justify-center bg-zinc-100 rounded-lg text-sm font-bold">
                                        {i + 1}
                                    </span>
                    ...
                    <div className="bg-gradient-to-br from-zinc-900 to-zinc-800 rounded-2xl p-5 text-white">
                        ...
                        <button onClick={onBackToPos} className="mt-6 block w-full text-center bg-white text-zinc-900 py-3 rounded-xl font-bold hover:bg-zinc-100 transition-colors">
                            Go to POS
                        </button>
                    </div>
                </div>
                    ...
                    <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl animate-in fade-in zoom-in duration-200 max-h-[80vh] flex flex-col">
                        ...
                        <div className="flex-1 overflow-y-auto p-5 space-y-3">
                            {reminders.filter(r => r.status === 'pending').length === 0 ? (
                                ...
                                reminders.filter(r => r.status === 'pending').map(reminder => {
                                    ...
                                    return (
                                        <div key={reminder.id} className={`p-4 rounded-xl border ${isOverdue ? 'bg-red-50 border-red-200' : 'bg-zinc-50 border-zinc-200'}`}>
                                            ...
                                            <div className="flex gap-2">
                                                ...
                                                <button
                                                    ...
                                                    className="px-3 py-2 bg-zinc-200 hover:bg-zinc-300 rounded-lg text-xs font-medium"
                                                >
                                                    +7 Days
                                                </button>
                                                <button
                        ...
                        <div className="p-5 border-t border-zinc-200">
                            <button
                                ...
                                className="w-full bg-zinc-100 text-zinc-700 py-3 rounded-xl font-bold hover:bg-zinc-200 transition-colors"
                            >
                                Close
                            </button>
                        </div>
